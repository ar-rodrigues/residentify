...
export default function CameraCapture({ onCapture, onCancel }) {
  ...
  const startCamera = useCallback(async (facing = facingMode) => {
    if (!isCameraAvailable()) {
      setError(
        t("qrCodes.camera.errors.cameraNotAvailable")
      );
      return;
    }
    ...
    if (!isSecureContext()) {
      setError(
        t("qrCodes.camera.errors.secureConnectionRequired")
      );
      return;
    }
    ...
    try {
      ...
      let errorMessage = t("qrCodes.camera.errors.accessError");

      if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
        errorMessage = t("qrCodes.camera.errors.permissionDenied");
      } else if (err.name === "NotFoundError" || err.name === "DevicesNotFoundError") {
        errorMessage = t("qrCodes.camera.errors.noCameraFound");
      } else if (err.name === "NotReadableError" || err.name === "TrackStartError") {
        errorMessage = t("qrCodes.camera.errors.cameraInUse");
      } else if (err.name === "OverconstrainedError") {
        errorMessage = t("qrCodes.camera.errors.constraintsError");
        ...
        try {
          ...
          errorMessage = t("qrCodes.camera.errors.accessFailed");
        }
      }
  ...
  return (
    <div className="w-full h-full" style={{ height: "100%" }}>
      {error && (
        <Alert
          title={t("qrCodes.camera.errorTitle")}
          description={error}
          type="error"
          showIcon
          closable
        ...
        {!isStreaming && !error && streamRef.current && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/90 z-10">
            <div className="text-center text-white">
              ...
              <Text className="text-white">{t("qrCodes.camera.startingCamera")}</Text>
            </div>
          </div>
        )}
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/70 to-transparent p-8 z-10">
            <div className="flex items-center justify-center gap-6">
              ...
              <Button
                ...
                title={t("qrCodes.camera.switchCamera")}
