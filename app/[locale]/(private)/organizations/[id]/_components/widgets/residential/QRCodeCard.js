...
export default function QRCodeCard({ qrCode, onClose }) {
  ...
  return (
    <Card
      title={
        <Space>
          ...
          <span>{t("qrCodes.card.title")}</span>
        </Space>
      }
      extra={
        onClose && (
    ...
    >
      <Space orientation="vertical" size="large" className="w-full">
        ...
        {/* Visitor Info (if validated) */}
        {qrCode.visitor_name && (
          <div className="border-t pt-4">
            <Title level={5} className="mb-4">
              {t("qrCodes.card.visitorInfo")}
            </Title>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <Text strong className="block mb-1">
                    {t("qrCodes.card.visitorName")}
                  </Text>
                  <Text>{qrCode.visitor_name}</Text>
                </div>
              </div>

              {qrCode.visitor_id && (
                <div className="flex items-start gap-3">
                  <RiIdCardLine className="text-xl text-gray-500 mt-1" />
                  <div>
                    <Text strong className="block mb-1">
                      {t("qrCodes.card.visitorId")}
                    </Text>
                    <Text>{qrCode.visitor_id}</Text>
                  </div>
                </div>
              )}
            </Space>
          ...
          <Space orientation="vertical" size="middle" className="w-full">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <Text strong className="block mb-1">
                  {qrCode.is_used ? t("qrCodes.card.validatedAt") : t("qrCodes.card.expiresAt")}
                </Text>
                <Text>
                  {qrCode.validated_at
                    ? formatDateDDMMYYYY(qrCode.validated_at)
                    : qrCode.expires_at
                    ? formatDateDDMMYYYY(qrCode.expires_at)
                    : t("qrCodes.card.notSpecified")}
                </Text>
              </div>
            </div>
