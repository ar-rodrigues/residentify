...
export default function ChatMessage({ item, type, onClick }) {
  ...
  const formatTime = (timestamp) => {
    ...
    if (diffDays === 0) {
      ...
      return `Ayer a las ${date.format("HH:mm")}`;
    } else if (diffDays < 7) {
      return date.format("dddd [a las] HH:mm");
    } else {
    ...
    switch (type) {
        ...
        return (
          <div>
            <Text strong className="block mb-1">
              {item.visitor_name
                ? `Código QR para ${item.visitor_name}`
                : "Código QR creado"}
            </Text>
            <Text type="secondary" className="text-sm">
              {item.visitor_name && item.visitor_id && `${item.visitor_id} • `}
              {item.expires_at
                ? `Expira: ${formatDateDDMMYYYY(item.expires_at)}`
                : item.created_at
                ? `Creado: ${formatDateDDMMYYYY(item.created_at)}`
                : ""}
            </Text>
          </div>
        );
      ...
      case "access_exit":
        return (
          <div>
            <Text strong className="block mb-1">
              {item.visitor_name || item.qr_code?.visitor_name || "Visitante"}{" "}
              {type === "access_entry" ? "entró" : "salió"}
            </Text>
            <Text type="secondary" className="text-sm">
              {item.scanned_by_name &&
                `Escaneado por ${item.scanned_by_name} • `}
